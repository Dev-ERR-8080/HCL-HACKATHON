...
const Hotels = () => {
  ...
  const [sortOption, setSortOption] = useState('popularity'); // popularity, price_asc, price_desc, rating_desc
  ...
  const filteredHotels = useMemo(() => {
    ...
    } // Popularity default is original API order or custom ID order

    return result;
  }, [searchQuery, selectedRatings, selectedPriceRange, selectedAmenities, sortOption]);
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">{filteredHotels.length} {filteredHotels.length === 1 ? 'property' : 'properties'} found</p>
          </div>
          ...
          <div className={`md:col-span-1 md:sticky md:top-24 z-40 md:z-auto ${isMobileFiltersOpen ? 'fixed inset-0 bg-white z-50 p-6 overflow-y-auto' : ''}`}>
            {isMobileFiltersOpen && (
              <div className="flex justify-between items-center mb-6 md:hidden">
                ...
                <button onClick={() => setIsMobileFiltersOpen(false)} className="p-2 border rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
              </div>
            )}
          ...
          <div className="md:col-span-3 flex flex-col gap-6">
            {filteredHotels.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-12 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-slate-500">We couldn't find any hotels matching your current filters. Try adjusting them.</p>
                <button 
                  onClick={() => {
                    setSearchQuery('');
                    setSelectedRatings([]);
